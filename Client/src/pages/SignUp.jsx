...
import { BiLogIn, BiUser, BiUserPlus } from "react-icons/bi";
...
const SignUp = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        const res = await API.post("/users/sign-up", {name,email, password, ComfirmPassword})
        if(res.data.success){
        toast.success("verification email sent")
        navigate("/verify-email")
        setData({name:"",email:"", password:"", ComfirmPassword:""})
        }
        }
        catch(error){
    ...
    <div className="grid place-items-center py-16">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-lg w-full max-w-md p-6 md:p-8">
        ...
        <div className="flex justify-center mb-3">
          <BiUserPlus size={46} className="text-gray-500" />
        </div>
